...
当用户切换角色时，系统会：
1. 清空当前会话的对话历史（内存）
2. 加载该角色的历史聊天记录（从 `logs/chat_{role_id}.txt`）
3. 加载该角色的存档六轴状态（从 `save/state_{role_id}.json`）
4. **重置引擎**（`self.engine = None`），确保下次对话使用正确的角色设定
5. 重置所有调试标签页的输入/输出内容

**为什么需要重置引擎？**
引擎初始化时会加载角色设定（`npc_*.md`），如果切换角色时不重置引擎，调试窗口中的角色设定会保留旧角色的内容。
