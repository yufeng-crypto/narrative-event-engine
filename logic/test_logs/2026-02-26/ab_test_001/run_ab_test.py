...
# 事件卡触发轮次（至少2轮）
EVENT_TRIGGER_ROUNDS = [4, 9]  # 第4轮和第9轮触发事件卡
...
def call_direct_llm(user_input: str, history: list, npc_context: str) -> str:
    ...
    messages = [
        ...
        {"role": "system", "content": f"当前六轴状态: Intimacy={character['axes']['Intimacy']}, Risk={character['axes']['Risk']}, Info={character['axes']['Info']}, Action={character['axes']['Action']}, Rel={character['axes']['Rel']}, Growth={character['axes']['Growth']}"},
        {"role": "system", "content": f"对话历史:\n{history_str}"},
        {"role": "user", "content": user_input}
    ]
    
    # 合并多个 system 消息（MiniMax API 不支持多个 system）
    messages = eng.merge_system_messages(messages)
